...
| Search the diagram | **Ctrl+F** (also the "🔍 検索" toolbar button or the Edit menu). Typing selects the first match and centres it, and while searching **every match glows yellow and the current one orange** (the glow never reaches exported images); **Enter goes to the next, Shift+Enter to the previous** (F3 / Shift+F3 work too). Past the last match it says it is continuing from the top and wraps around. Full-/half-width and upper/lower case are ignored, and collapsed topics are expanded to show a match |
| Resize the side panes | **Drag the divider** between the canvas and the left pane or the right panel (double-click it to restore the default; with it focused, ← / → adjust it too). Widths are remembered between runs, and the canvas always keeps a minimum width. When the right panel has more fields than fit, only the panel scrolls |
